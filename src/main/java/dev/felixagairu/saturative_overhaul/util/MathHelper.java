...
public class MathHelper {
    ...
    public static int generateRandom(int base, int low, int high) {
        if (low > high) {
            int tmp = low; low = high; high = tmp;
        }
        float min = base * low;
        float max = base * high;
        return (int) (min + (max - min) * (float) Math.random());
    }

    public static int clampFoodLevel(int inFoodLevel) {
        return Math.clamp(inFoodLevel, 0, ConfigHelper.thresholdMax);
    }
    public static float clampFoodLevel(float inFoodLevel) {
        return Math.clamp(inFoodLevel, 0, ConfigHelper.thresholdMax);
    }
}
